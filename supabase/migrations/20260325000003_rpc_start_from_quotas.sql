-- Fix: start the query from search_quotas so ALL users are returned,
-- even those who don't have a profile row yet.
...
BEGIN
  ...
  SELECT
    q.user_id,
    COALESCE(NULLIF(p.full_name, ''), NULLIF(p.email, ''), q.user_id::text)  AS full_name,
